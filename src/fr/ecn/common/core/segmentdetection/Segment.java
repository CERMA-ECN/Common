...
//	}
	...
	public double computeStartEndPoints(){
		...
		double meanX = 0;
		double meanY = 0;
		double Xmin = this.points.get(0).getX();
		double Xmax = this.points.get(0).getX();
		double Ymin = this.points.get(0).getY();
		double Ymax = this.points.get(0).getY();
		...
		this.startPoint = new Point(meanX - Math.cos(theta)*r/2, meanY - Math.sin(theta)*r/2);
		this.endPoint   = new Point(meanX + Math.cos(theta)*r/2, meanY + Math.sin(theta)*r/2);
	...
	public double[] computeParams() {
		...
		if ((endPoint.getX() - startPoint.getX()) != 0) {
			a = (endPoint.getY() - startPoint.getY())/(endPoint.getX() - startPoint.getX());
			b = endPoint.getY() - endPoint.getX()*a;
		} else {
			a = 1;
			b = endPoint.getX();
			vertical = 1;
