 ...
 *
 * @author Simon Horne
 * @author Leo COLLET, Ecole Centrale Nantes
 * From http://homepages.inf.ed.ac.uk/rbf/HIPR2/flatjavasrc/GaussianSmooth.java
 */
public class GaussianSmooth extends Thread {
	...
	public static double gaussianDiscrete2D(double theta, int x, int y){
	    ...
	    for(double ySubPixel = y - 0.5; ySubPixel < y + 0.55; ySubPixel += 0.1){
	    	for(double xSubPixel = x - 0.5; xSubPixel < x + 0.55; xSubPixel += 0.1){
	    		g = g + ((1/(2*Math.PI*theta*theta)) * Math.pow(Math.E,-(xSubPixel*xSubPixel+ySubPixel*ySubPixel)/(2*theta*theta)));
	    	}
	    }
	    g = g/121;
	...
	public static double [][] gaussian2D(double theta, int size){
	   ...
	   *
	   * @param		input	2D array representing the input image
	   ...
	   */
	public static double [][] smooth(double [][] input, int w, int h, int ks, double theta){
	    double [][] gaussianKernel = new double [ks][ks];
	    double [][] output = new double [w][h];
	    gaussianKernel = gaussian2D(theta,ks);
	    output = Convolution.convolution2DPadded(input,w,h,gaussianKernel,ks,ks);
	    return output;
	}
	  /**
	   * The same method, with an int[][] input image.
	   */
	public static int[][] smooth(int[][] input, int w, int h, int ks, double theta){
	    double [][] gaussianKernel = new double [ks][ks];
	...
	public static int [] smooth_image(int [] input, int w, int h, int ks, double theta){
	    ...
	    for(int j=0;j<h;++j){
	    	for(int i=0;i<w;++i){
	    		input2D[i][j] = Color.red(input[j*w+i]);
	    	}
	    }
	    ...
	    for(int i=0;i<output1D.length;++i){
	    	...
	    	output[i] = Color.rgb(grey, grey, grey);
	    }
	    return output;
	}
